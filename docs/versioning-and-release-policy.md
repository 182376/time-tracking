...
如果范围内只有小范围修复、回归修复、构建修复或非行为级 UI 微调，才使用 `PATCH`。
...
4. 在本地运行 `npm run release:check`，完成发布前验证。
5. 将准备发布所需提交推送到远端；发布提交信息统一使用 `release: vX.Y.Z`。
6. 推送对应的 `vX.Y.Z` 版本 tag，自动触发 GitHub Actions 的 `Publish Release` 工作流；必要时也可手动触发并输入同一版本号。
7. 由工作流生成 release notes、构建安装包与 `latest.json`。
8. 由工作流发布 GitHub Release。
9. 由工作流更新 updater 通道。
