...
- [ ] `0.6.6` release commit 尚未推送。
- [ ] `v0.6.6` tag 尚未推送。
- [ ] 等待当前电脑升级到 `0.6.6` 后确认数据正常。
- [ ] 等待另一台已安装 Time Tracker 的电脑升级到 `0.6.6` 后确认数据正常。
- [ ] 开始 `0.6.7` 完全简并前，至少在 `0.6.6` 内导出一份新的结构化 `.zip` 备份作为临时回滚点。
- [ ] `0.6.7` 完全简并版本尚未开始实施。
...
- [ ] 当前电脑已运行过 `0.6.6`。
- [ ] 另一台已安装电脑已运行过 `0.6.6`。
- [ ] 所有需要保留的数据都已在 `0.6.6` 中正常显示。
- [ ] 开始完全简并前，已导出至少一份 `0.6.6` 结构化 `.zip` 临时回滚备份。
